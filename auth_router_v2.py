"""Auth routes — login, logout, register, account, audit log endpoints."""
import logging
...
def get_current_session(request: Request) -> Optional[dict]:
    ...
    if not token:
        return None
    return get_auth_service().validate_session(token)
...
async def login_submit(request: Request, req: LoginRequest):
    ...
    if session:
        ...
        response.set_cookie(
            key=SESSION_COOKIE_NAME, value=session["token"],
...
async def register_user(request: Request, req: RegisterRequest):
    ip = request.client.host if request.client else "unknown"
    success, message = get_auth_service().register_user(
        req.username, req.password, req.invite_code, ip_address=ip
    )
    if success:
...
async def admin_reset_password(request: Request, req: ResetPasswordRequest):
    ...
    ip = request.client.host if request.client else "unknown"
    success, message = get_auth_service().reset_password(
        req.username, req.new_password, session["username"], ip_address=ip
    )
    if success:
        return {"success": True, "message": message}
    return JSONResponse({"success": False, "error": message}, status_code=400)
...
async def audit_log_page(request: Request):
    ...
    return templates.TemplateResponse("audit_log.html", {
        ...
        "event_types": EVENT_TYPES, "stats": stats,
    })
...
async def audit_log_query(
    ...
    entries, total = audit.query(
        event_type=event_type, severity=severity, username=username,
        search_text=search, start_date=start_date, end_date=end_date,
...
async def audit_log_export(request: Request, format: str = Query("json")):
    ...
    if format == "csv":
        ...
        if entries:
            ...
            writer.writeheader()
            for e in entries:
                writer.writerow({k: str(v) if not isinstance(v, str) else v for k, v in e.items()})
